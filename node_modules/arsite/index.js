...
var Promise = require('bluebird') // Promises
    , _ = require('lodash')
    , arfunc = require('arfunc') // ActiveRules Functionality
    , nugget = require('nugget') // ActiveRules Functionality
;
...
function middleware(app, settings) {
    ...
    var defaultSiteAlias = settings.defaultSiteAlias;

    return function (conn) {

        hostFilePath = settings.configDir + '/hosts/' + conn.host + settings.jsonExt;

        // Verify the host file exists
        return arfunc.fileStatus(hostFilePath)
            .then(function(hostStatus) {
                ...
                if(hostStatus === 'file') {
                    ...
                    // load host JSON
                    return nugget.loadCoreSchemaObject(hostFilePath, '/schema/activerules/hostConfig.json')
                        .then(function(hostConfig) {

                            // Does the host have a site defined?
                            if(typeof hostConfig.site !== 'undefined') {

                                siteFilePath = settings.configDir + '/sites/' + hostConfig.site + settings.jsonExt;
                            }
                            else {
                                // If no site is specfied but the hostname is supported the host is parked.
                                siteFilePath = settings.configDir + '/sites/' + defaultSiteAlias + settings.jsonExt;
                            }
                            // Verify the site file exists
                            return arfunc.fileStatus(siteFilePath)
                                .then(function(siteStatus) {
                                    ...
                                    if(siteStatus === 'file') {
                                        ...
                                        // Load Site JSON
                                        return arfunc.loadJSON(siteFilePath)
